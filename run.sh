...
# Keep the compiled stylesheet in step with styles/*.css -- rebuilt
# here when it has fallen behind, so `npm run css` is not something
# anyone has to remember. Never blocks startup (see the script).
